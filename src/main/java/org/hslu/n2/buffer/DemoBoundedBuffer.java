...
public final class DemoBoundedBuffer {
    ...
    public static void main(final String args[]) throws InterruptedException {
        ...
        final Thread tp1 = new Thread(p1, "Prod A");
        tp1.start();
        final Thread tp2 = new Thread(p2, "Prod B");
        tp2.start();
        final Thread tp3 = new Thread(p3, "Prod C");
        tp3.start();
        final Thread tp4 = new Thread(p4, "Prod D");
        tp4.start();
        final Thread tc1 = new Thread(c1, "Cons A");
        tc1.start();
        try {
            tp1.join();
            tp2.join();
